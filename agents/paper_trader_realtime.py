#!/usr/bin/env python3
"""
KingK Paper Trader - REAL-TIME VERSION
Uses real-time prices for trade execution, not 4h candle closes.
"""
import sys, os, time
...
try:
    ...
    print("✅ Loaded mixed portfolio config (Real-time version)")
except ImportError:
    # Fallback to default config
    from config.settings import *
    print("⚠️  Using default config (no mixed portfolio)")

PORTFOLIO_FILE = Path(__file__).parent.parent / "logs" / "paper_portfolio_realtime.json"
LOG_FILE       = Path(__file__).parent.parent / "logs" / "paper_trades_realtime.log"
...
def get_signal_with_realtime_price(symbol_side: str) -> dict | None:
    """
    Get signal for a symbol+side combination.
    Uses real-time ticker price for execution, not 4h candle close.
    """
    import pandas as pd
    ...
    interval = cfg.get("interval", "240")  # Default to 4h for strategy logic
    
    # Get REAL-TIME price for trade execution
    try:
        ticker = get_ticker(symbol)
        realtime_price = float(ticker["lastPrice"])
        log(f"  {symbol_side}: Real-time price = ${realtime_price}")
    except Exception as e:
        log(f"  {symbol_side} ❌ Failed to get real-time price: {e}")
        return None
    
    # Get klines for strategy logic (uses 4h candles for signal generation)
    ...
    return {
        "symbol": symbol,
        "side": side,
        "strategy_price": float(last["close"]),  # Price from strategy (4h close)
        "realtime_price": realtime_price,        # Real-time price for execution
...
def run_paper_trader_realtime():
    p = load_portfolio()
    log("=" * 60)
    
    if TRADING_MODE == "testnet" and TESTNET_ENABLED:
        log("⚠️  TRADING MODE: TESTNET (Real-time prices, Bybit demo)")
    else:
        log("📋 KingK Paper Trader - REAL-TIME PRICES (70% Long / 30% Short)")
    ...
    for i, symbol_side in enumerate(symbol_sides):
        ...
        data = safe_call("bybit", get_signal_with_realtime_price, symbol_side)
        if data is None:
        ...
        side = data["side"]
        cfg = data["cfg"]
        strategy_price = data["strategy_price"]
        ...
        price_diff_pct = (realtime_price - strategy_price) / strategy_price * 100
        if abs(price_diff_pct) > 0.5:  # More than 0.5% difference
            log(f"  {symbol_side}: Strategy price ${strategy_price} vs Real-time ${realtime_price} ({price_diff_pct:+.2f}%)")
        
        # Use REAL-TIME price for all calculations
        ...
        if symbol_side not in p["positions"] and p["cash"] >= alloc:
            if (side == "long" and entry_signal == 1) or (side == "short" and entry_signal == -1):
                # Calculate TP/SL based on side using REAL-TIME price
                if side == "long":
                ...
                # Handle entry
                handle_entry_realtime(symbol_side, symbol, side, qty, price, tp_price, sl_price, 
                                   module_name, p, alloc)
...
def handle_entry_realtime(symbol_side, symbol, side, qty, price, tp_price, sl_price, module_name, portfolio, alloc):
    """Handle entry logging and portfolio update with real-time prices."""
    if TRADING_MODE == "testnet" and TESTNET_ENABLED:
        log(f"\n⚠️  TESTNET ORDER PENDING CONFIRMATION")
        log(f"    Symbol: {symbol} | Side: {side.upper()} | Qty: {qty} | Real-time Price: ${price}")
